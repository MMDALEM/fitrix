...
async function generateProgram({ intake, metrics }) {
  ...
  const system = [
    ...
    "- برای هر روز ۵ تا ۷ حرکت با ست/تکرار/استراحتِ دقیق و نکته‌ی تکنیکی بده. حرکات با محل تمرین و آسیب‌های فرد سازگار باشند.",
    "- تیپ بدنی را جدی بگیر: اکتومورف → حجم و کالریِ بالاتر، استراحتِ بیشتر؛ اندومورف → تراکمِ بالاتر و کاردیوی بیشتر؛ مزومورف → متعادل.",
    "- در بخشِ تغذیه، **برای هر وعده کالریِ تقریبی را در فیلد calories بنویس** و مجموعِ وعده‌ها نزدیکِ کالریِ هدف باشد. غذاها واقعی، مقرون‌به‌صرفه و در دسترسِ ایران باشند (نه فقط مکمل).",
    "- در supplements فقط نامِ عمومیِ مکمل بده (پروتئین وی، کراتین، گینر، بی سی ای ای، مولتی ویتامین، امگا ۳، گلوتامین) و با بودجه‌ی فرد بخوان؛ اول اقلامِ ضروری.",
  ...
  const user = "مشخصاتِ کاربر:\n" + profileText(intake, metrics) + "\n\nحالا برنامه‌ی کامل را طراحی کن.";
  const out = await callAIJson({ system, user, maxTokens: 3000 });
